...
def migrar_dados_destino(apps, schema_editor):
    ...
    Viagem = apps.get_model('transportes', 'Viagem')
    ...
    destinos_existentes = (
        Viagem.objects
        .annotate(destino_viagem_upper=Upper('destino_viagem'))
    ...
    for viagem in Viagem.objects.all():
        ...
        destino_upper = viagem.destino_viagem.upper() if viagem.destino_viagem else None
        
        if destino_upper and destino_upper in destino_map:
            viagem.destino =destino_map[destino_upper]
            viagem.save(update_fields=['destino'])
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('transportes', '0007_viagem_destino'), # <-- MUDAR PARA SUA ÚLTIMA MIGRATION
    ]
