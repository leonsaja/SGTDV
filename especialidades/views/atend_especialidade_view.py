...
"""def search_pacientes(request):
    ...
    View para buscar pacientes via AJAX.
 
    term = request.GET.get('term', '')
    especialidade_id = request.GET.get('especialidade_id', None)

    pacientes_list = []
    
    if especialidade_id:
        # Busca pacientes_especialidade filtrando pela especialidade e termo de busca
        pacientes_especialidade = PacienteEspecialidade.objects.filter(
            especialidade_id=especialidade_id).filter(
            Q(paciente__nome_completo__icontains=term) | Q(paciente__cns__icontains=term)
        ).select_related('paciente', 'especialidade','procedimento').order_by('paciente__nome_completo')

        for pe in pacientes_especialidade[:20]:
            paciente = pe.paciente
            procedimento = pe.procedimento
            print('procedimento',procedimento)
            classificacao = pe.get_classificacao_display()

            # Constrói o texto que será exibido no dropdown do Select2
            option_text = (
                f"{paciente.nome_completo} (DN: {paciente.dt_nascimento.strftime('%d/%m/%Y')} - CNS: {paciente.cns}) "
                f"- Procedimento: {procedimento if procedimento else 'N/A'} - Classificação: {classificacao}"
            )

            pacientes_list.append({
                'id': pe.pk, # É melhor retornar o ID do PacienteEspecialidade para a relação
                'text': option_text,
            })

    return JsonResponse({'results': pacientes_list})
"""